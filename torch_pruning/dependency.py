...
class _ConcatIndexTransform(object):
    ...
    def __call__(self, idxs):
        if self.reverse:
            new_idxs = [
                i - self.offset[0] for i in idxs if (i >= self.offset[0] and i < self.offset[1])
            ]
        else:
...
class Node(object):
    ...
    def node_name(self):
        return (
            "%s (%s)" % (self._node_name, str(self.module))
            if self._node_name is not None
            else str(self.module)
        )
...
class Dependency(object):
    def __init__(
        self, trigger, handler, broken_node: Node, index_transform: typing.Callable = None
    ):
...
class PruningPlan(object):
    ...
    Args:
        ...
        module_to_name (dict): mapping nn.module to a readable name.
        It will be filled by DependencyGraph.
    """
...
class DependencyGraph(object):
    ...
    for t1 in HANDLER.keys():
        for t2 in HANDLER.keys():
            OUTPUT_NODE_RULES[(t1, t2)] = (
                HANDLER[t1][1],
                HANDLER[t2][0],
            )  # change in_channels of output layer
            INPUT_NODE_RULES[(t1, t2)] = (
                HANDLER[t1][0],
                HANDLER[t2][1],
    ...
    def build_dependency(
        ...
        self.module_to_node = self._obtain_forward_graph(
    ...
    def register_customized_layer(
        self, layer_type, in_ch_pruning_fn, out_ch_pruning_fn, get_in_ch_fn, get_out_ch_fn
    ):
    ...
    def _obtain_forward_graph(self, model, example_inputs, output_transform):
        ...
        hooks = [
            m.register_forward_hook(_record_module_grad_fn)
        ...
        def _build_graph(grad_fn):
            ...
            if module is None:
                if not hasattr(grad_fn, "name"):
                    ...
                    if self.verbose:
                        print(
                            "[Warning] Unrecognized operation: %s. \
                             It will be treated as element-wise op"
                            % (str(grad_fn))
                        )
            ...
            if hasattr(grad_fn, "next_functions"):
                for f in grad_fn.next_functions:
                    if f[0] is not None:
                        if (
                            hasattr(f[0], "name") and "accumulategrad" in f[0].name().lower()
                        ):  # skip leaf variables
                            continue
